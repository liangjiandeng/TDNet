...
class panbranch(nn.Module):
    def __init__(self, conv=default_conv):
        ...
        self.conv_2 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1,
                                bias=True)
        self.conv_output1 = nn.Conv2d(in_channels=64, out_channels=4, kernel_size=3, stride=1, padding=1,
                                      bias=True)
...
class mutiscale_net8(nn.Module):
    def __init__(self):
        super(mutiscale_net8, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=8, out_channels=60, kernel_size=7, stride=1, padding=3,
                               bias=True)
        ...
        self.conv5 = nn.Conv2d(in_channels=30, out_channels=4, kernel_size=5, stride=1, padding=2,
                               bias=True)
...
class panbranch2(nn.Module):
    def __init__(self, conv=default_conv):
        super(panbranch2, self).__init__()
        channel = 8
        self.conv_input = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1,
                                    bias=True)
        self.conv_1 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1,
                                bias=True)
        self.conv_2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1,
                                bias=True)
        self.conv_output1 = nn.Conv2d(in_channels=32, out_channels=4, kernel_size=3, stride=1, padding=1,
                                      bias=True)
        self.downscale = nn.MaxPool2d(kernel_size=2, stride=2)
        self.relu = nn.ReLU(inplace=True)
        self.conv_12 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1,
                                 bias=True)
        self.conv_22 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1,
                                 bias=True)
        init_weights(self.conv_input, self.conv_1, self.conv_2, self.conv_output1, self.downscale, self.conv_12,
                     self.conv_22)
...
class TDNet_band42(nn.Module):
    def __init__(self):
        super(TDNet_band42, self).__init__()

        spectral_num = 4
        upscale = 2
        self.pan_branch = panbranch2()
        ...
        self.mulnet = mutiscale_net82()
        self.mulnet2 = mutiscale_net82()

        init_weights(self.mulnet, self.mulnet2, self.conv_up, self.subpixel_up, self.conv_up2, self.subpixel_up2, self.downscale_pan)
